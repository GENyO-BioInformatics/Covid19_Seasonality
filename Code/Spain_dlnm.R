...
for(j in names(list_reg)){
    ...
    modelp2 <- gam(datReg$Re ~ cb_temp + cb_SI + datReg$variants + vacI + int2 + ns(datReg$days, df = 2), 
        family = quasipoisson())
    #Predictions
    pred.p1 <- crosspred(cb_temp, modelp1, cen = mean(datReg$Temperature), 
        at = min(datReg$Temperature):max(datReg$Temperature))
    pred.p2 <- crosspred(cb_temp, modelp2, cen = mean(datReg$Temperature), 
        at = min(datReg$Temperature):max(datReg$Temperature))
    #New dataframe
...
setwd("Figures")
ggsave(
    filename = "Supplementary_Figure_S7.jpeg",
    plot = dlnm_Spain_fig,
    scale = 1,
    width = 1400,
    height  = 1200,
...
Temperatures <- matrix(NA, nrow = length(list_reg$Andalusia$Temperature), length(list_reg))
colnames(Temperatures) <- names(list_reg)
for (reg in names(list_reg)){
    Temperatures[,reg] <- list_reg[[reg]]$Temperature
}
tmean <- rowMeans(Temperatures)
bvar <- onebasis(tmean)
...
for(j in names(list_reg)){
    ...
    modelp2 <- gam(datReg$Re ~ cb_SH + cb_SI + datReg$variants + vacI + int2 + ns(datReg$days, df = 2), 
        family = quasipoisson())
    #Predictions
    pred.p1 <- crosspred(cb_SH, modelp1, cen = mean(datReg$SH), 
        at = min(datReg$SH):max(datReg$SH))
    pred.p2 <- crosspred(cb_SH, modelp2, cen = mean(datReg$SH), 
        at = min(datReg$SH):max(datReg$SH))
    #New dataframe
    RR <- c(pred.p1$allRRfit,pred.p2$allRRfit)
    RRlow <- c(pred.p1$allRRlow, pred.p2$allRRlow)
...
setwd("Figures")
ggsave(
    filename = "Supplementary_Figure_S9.jpeg",
    plot = dlnm_Spain_fig,
    scale = 1,
    width = 1400,
    height  = 1200,
...
SHs <- matrix(NA, nrow = length(list_reg$Andalusia$SH), length(list_reg))
...
setwd("Figures")
ggsave(
    filename = "Supplementary_Figure_S8.jpeg",
    plot = dlnm_meta_Spain_fig,
    scale = 1,
    width = 1400,
    height  = 1200,
